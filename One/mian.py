...
def get_page(page_num, paramets):
    url = f"http://match.yuanrenxue.com/api/match/1?page={page_num}&m={paramets}"
    # print(url)
    headers = {
...
def calculate_m_value():
    ...
    with open(r'D:\Code\JSReverse\One\atobJS.js', encoding='utf-8', mode='r') as f:
        JsData = f.read()

    psd = execjs.compile(JsData).call('get_m_value')
    print(psd["123"])
    print(psd["timestamp"])
    # psds = execjs.compile(jscode).call('get_m')
